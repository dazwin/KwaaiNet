...
Pre-built binaries for **v0.1.8** are attached to the [latest GitHub Release](https://github.com/Kwaai-AI-Lab/KwaaiNet/releases/latest) — no Rust or Go toolchain required.
...
**Latest Achievements:**
- ✅ **v0.1.8 Released** — fixed `p2pd` injection into platform archives (v0.1.7 missed it because cargo-dist stages extra-artifacts outside `core/`); uses `find` to locate `p2pd` regardless of staging path
